"""Portable command-line client for the OpenImage image API."""
...
def _normalize_base_url(value: str) -> str:
    ...
    if (
        ...
        raise ValueError("API 地址必须是无用户信息、无查询参数和 Fragment 的 HTTPS 443 地址。")
    return raw
...
class Settings:
    ...
    @classmethod
    def from_environment(cls, base_url: str | None = None) -> "Settings":
        api_key = os.environ.get("OPEN_IMAGE_API_KEY", "").strip()
        selected_url = base_url if base_url is not None else os.environ.get("OPEN_IMAGE_BASE_URL", "").strip()
        missing: list[str] = []
        if not selected_url:
            missing.append("OPEN_IMAGE_BASE_URL")
        if not api_key:
            missing.append("OPEN_IMAGE_API_KEY")
        if missing:
            raise RuntimeError(f"未设置配置: {', '.join(missing)}。")
        return cls(api_key, selected_url)
...
def _headers(settings: Settings) -> dict[str, str]:
    return {
        ...
        "User-Agent": "OpenImageCodexSkill/1.0",
    }
...
def _save_png(image_bytes: bytes, output_dir: Path) -> Path:
    ...
    path = output_dir / f"open-image-{secrets.token_hex(12)}.png"
...
def _task_location(headers: dict[str, str], settings: Settings) -> str:
    ...
    if (
        location.startswith("//")
        or location_parts.scheme
        or location_parts.netloc
...
def build_edit_request(
    ...
    boundary = f"----OpenImage{secrets.token_hex(16)}"
    chunks: list[bytes] = []
...
def generate(
    ...
    settings = Settings.from_environment(base_url)
    payload = build_generation_request(prompt, model, size, quality, count)
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="OpenImage 生图客户端")
    parser.add_argument("--base-url", help="OpenAI-compatible API 根地址")
    subcommands = parser.add_subparsers(dest="command", required=True)
    for command in ("generate", "edit", "text"):
        current = subcommands.add_parser(command)
        current.add_argument("--base-url", default=argparse.SUPPRESS, help="OpenAI-compatible API 根地址")
    ...
    batch_parser.add_argument("--base-url", default=argparse.SUPPRESS, help="OpenAI-compatible API 根地址")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if arguments.command == "batch":
            results = generate_batch(arguments.prompt, arguments.model, arguments.size, arguments.quality, arguments.output_dir, arguments.base_url)
            paths, errors = _batch_result_data(results)
            errors = _take_retry_notices() + errors
            exit_code = _print_batch_results(results)
        else:
            if arguments.command == "generate":
                paths = generate(arguments.prompt, arguments.model, arguments.size, arguments.quality, arguments.count, arguments.output_dir, arguments.base_url)
            elif arguments.command == "edit":
                paths = edit(arguments.prompt, arguments.reference, arguments.model, arguments.size, arguments.quality, arguments.count, arguments.output_dir, arguments.base_url)
            else:
                prompt = build_text_prompt(arguments.text, arguments.description, arguments.language, arguments.position, arguments.style)
                paths = generate(prompt, arguments.model, arguments.size, arguments.quality, arguments.count, arguments.output_dir, arguments.base_url)
